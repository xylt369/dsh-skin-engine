window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 从取色结果派生「特效调色板」：所有光标特效的颜色都随背景图片变化，而不是写死
    // dark = 背景偏暗。暗背景用偏亮的色调、亮背景用偏暗的色调，保证在任何图片上都看得清
    function paletteOf(c) {
      const accent = c && c.accent ? c.accent : [86, 130, 246];
      const base = c && c.base ? c.base : [24, 26, 32];
      const dark = !c ? true : (typeof c.textLight === 'boolean' ? c.textLight : lum(base) < 0.55);
      return { accent: accent, base: base, dark: dark };
    }
    // 强调色的明暗变体：ratio 0 保留原色，ratio 1 偏向白（暗背景）/黑（亮背景）
    function accentTint(p, ratio) {
      return mix(p.accent, p.dark ? [255, 255, 255] : [0, 0, 0], ratio);
    }
    // 强调色的色相族：用于极光等多色效果
    function accentFamily(p, hueOffsets) {
      const hue = rgbToHsl(p.accent)[0];
      const s = p.dark ? 0.85 : 0.7;
      const l = p.dark ? 0.65 : 0.45;
      return hueOffsets.map(function (o) { return hslToRgb(hue + o, s, l); });
    }
    ...
    function drawQuantum(mx, my) {
      ...
      const main = paletteOf(STATE.colors).dark ? [255, 255, 255] : [18, 20, 26]; // 亮背景用深色准星
    ...
    const BUILTIN_PRESETS = {
      ...
      glow: {
        ...
        render: function (c) {
          const ctx = c.g;
          const p = paletteOf(c.colors);
          const core = accentTint(p, 0.72); // 亮背景→偏暗色调，暗背景→偏亮色调
          ctx.globalCompositeOperation = 'screen';
          let g = ctx.createRadialGradient(c.mx, c.my, 0, c.mx, c.my, Math.max(c.w, c.h) * 0.42);
          g.addColorStop(0, rgba(core, 0.30));
          g.addColorStop(0.35, rgba(core, 0.10));
          ...
          g.addColorStop(0, rgba(core, 0.16));
          g.addColorStop(1, rgba(core, 0));
          ctx.fillStyle = g;
          ctx.fillRect(0, 0, c.w, c.h);
        },
      ...
      ripple: {
        ...
        onEnter: function () {
          // 选中即给反馈：在光标处播一圈初始波纹
          const cx = anim.mx * window.innerWidth;
          const cy = anim.my * window.innerHeight;
          for (let i = 0; i < 4; i++) {
            anim.rings.push({ x: cx + (Math.random() - 0.5) * 40, y: cy + (Math.random() - 0.5) * 40, r: 8, a: 0.5 });
          }
        },
        ...
        render: function (c) {
          ...
          for (let i = anim.rings.length - 1; i >= 0; i--) {
            ...
            ctx.globalCompositeOperation = 'screen';
            ctx.strokeStyle = rgba(ring, Math.max(0, r.a));
            ctx.lineWidth = 2;
            ctx.beginPath();
      ...
      particles: {
        ...
        onEnter: function () {
          ...
          const cx = anim.mx * window.innerWidth;
          const cy = anim.my * window.innerHeight;
          for (let i = 0; i < 24; i++) anim.parts.push(makePart(cx, cy));
        },
        onPointerDown: function (e) {
        ...
        render: function (c) {
          ...
          for (let i = anim.parts.length - 1; i >= 0; i--) {
            ...
            part.life -= c.dt;
            if (part.life <= 0) { anim.parts.splice(i, 1); continue; }
            part.x += part.vx * c.dt;
            part.y += part.vy * c.dt;
            part.vy += 30 * c.dt;
            const al = Math.max(0, Math.min(1, part.life / 0.9));
            ctx.globalCompositeOperation = 'screen';
            ctx.fillStyle = rgba(dot, al * 0.85);
            ctx.beginPath();
            ctx.arc(part.x, part.y, Math.max(0.5, part.size * al), 0, Math.PI * 2);
            ctx.fill();
          }
        },
      },
      aurora: {
        ...
        render: function (c) {
          const ctx = c.g;
          const p = paletteOf(c.colors);
          // 三条光带 = 强调色色相族（hue、+130°、+240°），随图片变化
          const family = accentFamily(p, [0, 130, 240]);
          ...
          for (let i = 0; i < anim.blobs.length; i++) {
            ...
            g.addColorStop(0, rgba(family[i], 0.18));
            g.addColorStop(1, rgba(family[i], 0));
            ctx.fillStyle = g;
            ctx.fillRect(0, 0, c.w, c.h);
          }
        },
      ...
      stars: {
        ...
        render: function (c) {
          ...
          for (let i = 0; i < anim.stars.length; i++) {
            ...
            ctx.fillStyle = rgba(tint, al * 0.8);
            ctx.beginPath();
