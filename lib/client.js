window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 平台协议版本：第三方预设通过 format: 1 声明兼容；引擎承诺向后兼容协议 v1
    ...
    function validateSpec(spec) {
      ...
      if (spec.format !== undefined && spec.format !== PLATFORM_FORMAT) return 'format 必须是 ' + PLATFORM_FORMAT + '（平台协议 v' + PLATFORM_FORMAT + '）';
    ...
    // 这是「现成的接口」：第三方预设作者不用碰插件内部，用下面这个对象即可
    // 探测引擎、注册预设、拿工具函数：
    //   const eng = window.__DSH_SKIN_ENGINE__;
    //   if (eng && eng.format === 1) eng.register({ id, name, desc, render });
    // 发布为 npm 包时，包加载后调用 eng.register(spec) 或直接写 window.__DSH_SKIN_PRESETS__。
    function registerPreset(spec) {
      const err = validateSpec(spec);
      if (err) return { ok: false, error: err };
      const reg = externalRegistry() || (window[EXTERNAL_REGISTRY_NAME] = {});
      reg[spec.id] = spec;
      setState({});
    ...
    function platformApi() {
      return {
        ...
        list: getAllPresets,
        remove: removeCustomPreset,
        isCustom: isCustomPreset,
        utils: { mix: mix, rgba: rgba, lum: lum, rgbToHsl: rgbToHsl, hslToRgb: hslToRgb, shiftHue: shiftHue },
      };
    }
    function ensurePlatformApi() {
      try {
        window.__DSH_SKIN_ENGINE__ = platformApi();
      } catch (e) { /* 非浏览器环境忽略 */ }
    }
                ...
                return h('div', {
                  ...
                  h('div', { className: 'dsh-skin-card-name' },
                    ...
                    isCustomPreset(key)
                      ...
                      : h('span', { className: 'dsh-skin-card-tag', 'data-kind': BUILTIN_PRESETS[key] ? 'builtin' : 'community' }, BUILTIN_PRESETS[key] ? '示例' : '社区'),
                  ),
...
.dsh-skin-card-tag[data-kind="community"]{background:var(--dsw-alias-state-business-tertiary);color:var(--dsw-alias-label-primary)}
