window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function loadSkin() {
      ...
      try {
        ...
        if (rawState) {
          ...
          const next = {};
          if (typeof saved.animation === 'string' && ANIMATIONS[saved.animation]) next.animation = saved.animation;
    ...
    const ANIMATIONS = {
      ...
      quantum: { name: '量子霓虹', desc: 'RGB 色散准星随速度拉伸旋转' },
      mercury: { name: '液态水银', desc: 'Verlet 质点链与果冻粘连拖尾' },
      stardust: { name: '星尘引力', desc: '星尘粒子被光标引力卷成涡流' },
      bauhaus: { name: '包豪斯网格', desc: '网格吸附方块，靠近按钮吸附边框' },
    };
    ...
    function syncBg() {
      ...
      // 模式切换：重置预置内部状态 + 管理 bauhaus 前台光标与果冻滤镜
      if (STATE.animation !== lastAnim) {
        ...
        if (STATE.animation === 'bauhaus') ensureBauhaus();
        else destroyBauhaus();
      }
      if (bgCanvas) {
        bgCanvas.style.filter = STATE.animation === 'mercury' ? 'blur(10px) contrast(20)' : 'none';
      }
    }
