...
上传一张背景图，插件会自动提取图片的主色、强调色和明暗，把整套 `--dsw-alias-*` 主题 token 换掉，再叠加跟随光标变化的动态背景，让整个应用立刻变成你的皮肤。

## 功能

- **上传背景图**：点击选择或拖拽 `jpg / png / webp / gif`，实时预览、即时生效
- **智能取色**：自动提取主色、强调色与明暗；面板、输入框、气泡、菜单、按钮等主题 token 全部跟随换肤，文字自动黑/白
- **6 种光标动态背景**：静态、光晕跟随、涟漪扩散、粒子拖尾、极光流动、星空视差
- **效果调节**：图片不透明度、面板通透度、背景模糊、暗化程度四个滑杆
- **入口**：侧边栏底部「🎨 换肤中心」按钮

> 皮肤状态目前是页面内存态：刷新页面后恢复默认；插件本身随 profile 常驻，无需每次重装。

## 安装

### 方式 A：npm 一键安装（适合分发给别人）
...
1. 在 `~/.dsh/profiles/web/package.json` 写入依赖和 `dsh.profile.bundles` 条目
2. 自动运行 `pnpm install`
3. 提示你重启 dsh

#### 手动配置

如果不想用脚本，编辑 `~/.dsh/profiles/web/package.json`，把下面两处合并进你现有的配置（**不要直接覆盖整个文件**）：
...
把整个 `dsh-skin-engine` 文件夹拷贝给对方，然后执行：
...
原理相同，只是 `package.json` 里的依赖写成 `file:` 绝对路径，无需 npm 发布。
...
| `--dsh-home <path>` | dsh 根目录，默认 `~/.dsh`，也可用 `DSH_HOME` 环境变量 |
...
- **取色**：图片上传后缩放到 64×64 画布，计算平均色作为 `base`、饱和度最高的像素作为 `accent`，再根据亮度决定文字黑/白。
- **动画**：六种效果都画在一个全屏 `canvas` 上，使用 `requestAnimationFrame` 驱动，指针事件监听随卸载清理。
- **清理**：样式标签、DOM 节点、事件监听、token 覆盖全部在 `ctx.effect` 的清理函数中撤销，插件卸载后不残留副作用。
